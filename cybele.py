...
def extdb_askards():
	...
	if os.path.isfile ('askards.db') == True and len(askards_db) == 0:
		print ('\n > Connecting with DB File')
		dbpath = "askards.db"
		conn = sqlite3.connect(dbpath)
		query = "SELECT * FROM askard_db"
		results = query_database(conn, query)
		print (' > Creating the local DB')
		for row in results:
			askards_db.append(row)
		conn.close()
		print (' > Task completed\n')
	elif os.path.isfile ('askards.db') == True and len(askards_db) > 0:	
		print ('The local database was allready created and have data!\n')
	else:
		print (random.choice(extmessages['trouble_short']) + " " + random.choice(extmessages['trouble_msg']) + " Internal error. Well done "+extauthor+"! Just Fix'it.\n")

#-------------------------------------------------
def list_askards_db():
	
	if askards_db and len(askards_db) > 0:
		for askard_id, askard in askards_db:
			print(" > " + str(askard_id))
			print(" "*3 + str(askard))
		print ("")
	else:
		print (random.choice(extmessages['trouble_short']) + " " + random.choice(extmessages['trouble_msg']) + " I cannot List what don't exist! Did you askards ?!\n")
