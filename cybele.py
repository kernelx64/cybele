...
topics = ["astronomy glossary","planets","planet orbit","orbits acronyms","asteroids","constelations","information about stars","distance of planets and from the sun",
		...
		"the world capitals","seasons of the year","play capitals","math game","constellations and elements game","linux command",
...
meaning_term = {
	...
	"askard": _spchar_[9:10]+" ASKard (noun): A non-screen production credit also knowed by 'minhas calinadas' verbal's that is usual on the end accompanie by the finger sometimes in my conversations with my friends or at some point to provoque a silence or a awkward one or a mindblowing usualy no more than half a minute at the end of the same.\nMore info in: www.adelinosaldanha.site/askards\n",
...
def find_answer(question,whatlist):
	...
	others_keys = ["country", "capital", "months", "seasons", "old_tech_term", "word meaning", "help"]
...
def mandb(dbname,dbtask,dbbegin,dbend):
	...
	conn = sqlitecloud.connect("sqlitecloud://cxuomo3ahz.g1.sqlite.cloud:8860/askardb.sqlite?apikey=0fEWazbBhdVtVHxJhtQQgv5uFzq4Y4IHG8i2Auu9K2I")
	zdb = []
	...
	if dbtask == 'limits':	
		if dbname == 'askardb':
			filter = "SELECT min(ask_id) , max(ask_id) FROM askardb"
			titvar = "askard ID"
		elif dbname == 'cybele':
...
def main():
	...
	aboutyou = managedb(aboutyou, checksum)
	#----------------------------
	if chkauth!=cybchk:
		print(kolor['DARK_RED'] + "\n " +_spchar_[1:2] + chr(32) + _title_ + kolor['OFF'] + ": " + hex(chkauth) + " - " + managedb(chkcyb, checksum))	
		exit()	
	#----------------------------
	drawart('art_cybele')
	#----------------------------
	...
	while True:
		...
		# days_until() gives' incorrect days in vorian online time. Verify. for next version
			...
			nversion = "I am "+_title_ +"'online' in the version "+version+" last updated in "+_revise_+" running for " + str(days_till_today.days) + " days."
			print (nversion + "\n")
		#------------------------------------------------
		...
		#------------------------------------------------
			...
			if node_name:
			  ...
			  print ('  Device : unidentified platform')
			print('     Name : ' + _title_)
