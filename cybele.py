...
"""
# Latitude and longitude of City for use with the command: set default gps for the sunset time for ex.
lat = 41.5454
lon = -8.4265
...
def extract_from_vorian():
	...
	try:
		...
		print("Error fetching the page: %s" % (e))
	except Exception as e:
...
def main():	
	...
	#----------------------------
	if chkcoor() == True:
		_poigps_=[lat_val,lon_val,0,0,0]
	else:
		modname = kdecode(seecoor, shift)
		mmodname = modname + "\n   I cannot execute properly. Exiting."
		...
		#world population
			...
			if len(sub) == 0:
				...
				if tvshow_share in webshare:
					print("Related to the digited name of the " + tvshow_share.upper() + " i have this link: \n > " + kdecode(webshare[tvshow_share],shift) + "\n")
					copy2clipboard(kdecode(webshare[tvshow_share],shift))
				else:
				...
				for tvshow, link in webshare.items():
					...
					print(" "*3 + kdecode(str(link),shift))
					if netchk == True:
						print(" : " + str(link_status(kdecode(link,shift))))
				print ("")
